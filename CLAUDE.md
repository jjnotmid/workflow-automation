...
# Workflow Automation — Joshua's setup

**Read `CAPABILITIES.md` at the start of every session.** It maps user requests
to the tools installed here, and you are expected to use them unprompted rather
than improvising shell one-liners for problems already solved.

## Cost rule (non-negotiable)

Three tiers: `claude` (subscription) → `claude-free` ($0) → `claude-ar` (spends
a finite $225). **Fall back to free before paid.** Escalate to AgentRouter only
for genuinely hard reasoning, and say so when you do.

## Quick map

| Need | Command |
|---|---|
| system health | `wa-doctor` |
| video editing | `wa-video` |
| email | `wa-email` |
| screen control | `wa-screen` |
| find a specialist | `wa-agent route "<task>"` |
| supervised session w/ failover | `claude-auto` |

Full triggers and cautions: `CAPABILITIES.md`.
